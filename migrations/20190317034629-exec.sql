...
DO $$
DECLARE 
 anime_cursor CURSOR FOR SELECT id, name FROM anime;
 rec RECORD;
 words VARCHAR(50)[];
 ngramm VARCHAR(3);
BEGIN  
 OPEN anime_cursor;
  LOOP
    ...
    EXIT WHEN NOT FOUND;
	words := regexp_split_to_array(rec.name, ' ');
       ...
       LOOP
        ...
        BEGIN
         ...
         EXCEPTION WHEN OTHERS THEN 
          BEGIN
           raise notice 'error occurs with row with id = %. anime.name = %. ngramm = %', rec.id, rec.name, ngramm;
          END;
        END; 
       END LOOP;
	 END LOOP;
